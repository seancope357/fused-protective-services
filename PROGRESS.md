...
| **DPS Licence Number** | 🟢 **Supplied** | `01766480`, supplied by Sean 2026-09-14; footer, schema, proposals and invoices carry it. The DPS-record street address is still owed. `docs/OPEN_QUESTIONS.md` §2. |
...
| **P0** | **Gold interactive-gradient decision** | Blocking the accessibility gate. A `--gradient-gold-brushed-ui` clamped at `#a1814c` for interactive faces; the decorative gradient stays as is. Reversible; the current ramp fails WCAG AA as a button face. (`--text-tertiary` → `#8f8a86` was approved and applied 2026-09-14.) | Cameron/Sean to approve the look |
...
| **P3** | **Paper gold labels on white** | The client-facing proposal and invoice papers set `.paper__motto` and `.paper__label` in a hard-coded `#a1814c` on white, about 3.2:1 (axe `color-contrast`, found by the seeded capture). A brand-colour decision, like the gold interactive gradient; print is the paper's main use. | Cameron/Sean to approve the look |
