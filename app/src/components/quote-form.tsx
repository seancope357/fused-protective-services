...
export function QuoteFields({ quote, clients, sites, clientId, defaultTaxRate }: { quote?: Quote | null; clients: Client[]; sites: Site[]; clientId?: string | null; defaultTaxRate?: number }) {
    ...
    return (
        <div className="form-grid">
            ...
            <Field id="officer_count" label="Officers"><input id="officer_count" name="officer_count" type="number" min={1} step={1} defaultValue={q?.officer_count ?? 2} required /></Field>
            <Field id="hours" label="Hours (total per officer)"><input id="hours" name="hours" type="number" min={0.5} step={0.5} defaultValue={q?.hours ?? 8} required /></Field>
            <Field id="bill_rate" label="Bill rate $/hr" hint="Defaults from the published rate card; edit for negotiated pricing."><input id="bill_rate" name="bill_rate" type="number" min={0} step={0.01} defaultValue={rateDollars} required /></Field>
            <Field id="tax_rate_pct" label={`${taxDefaults.label} %`}><input id="tax_rate_pct" name="tax_rate_pct" type="number" min={0} max={100} step={0.001} defaultValue={q?.tax_rate_pct ?? defaultTaxRate ?? taxDefaults.defaultRatePct} /></Field>
            <Field id="deposit_pct" label="Deposit %" hint="0 for none. A deposit invoice can be generated from the job."><input id="deposit_pct" name="deposit_pct" type="number" min={0} max={100} step={1} defaultValue={q?.deposit_pct ?? 0} /></Field>
            ...
            <Field id="ends_at" label="Coverage ends"><input id="ends_at" name="ends_at" type="datetime-local" defaultValue={isoToLocal(q?.ends_at)} /></Field>
            <Field id="notes" label="Internal notes" className="span-2"><textarea id="notes" name="notes" rows={4} defaultValue={q?.notes ?? ''} /></Field>
        </div>
    );
}
