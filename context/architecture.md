...
* **Stack:** Next.js 16 App Router (TypeScript, Turbopack, pnpm), Supabase (Postgres, Auth, RLS), Stripe, Resend, Twilio. Server components + server actions; the only client components are the login and MFA forms, the Security panel, a print button, and `components/nav.tsx` (current-page marking and the More sheet).
...
* **Mobile-first (SPEC-012):** `app.css` is written for the phone, with `min-width` layers at 600px (tablet) and 1024px (desktop). Below 1024px the frame (`components/shell.tsx`) is a sticky top bar, a bottom tab bar and a More sheet (native `<dialog>`); from 1024px it is the sidebar. Both navs render and CSS shows one. Lists use `components/data-table.tsx` (cards below 600px, a table above, explicit ARIA table roles); a screen's main action goes in `PageHead primary`, pinned above the tab bar on phones and tablets. Rules: `context/ui-standards.md` → Portal responsive rules.
